...
public class FairScheduler extends TaskScheduler {
  ...
  private class JobInitializer {
    ...
    private List<InitJob> jobInitQueue;
    ...
    public JobInitializer(Configuration conf, TaskTrackerManager ttm) {
      ...
      jobInitQueue = new ArrayList<InitJob>();
      JobInitManagerThread = new Thread(new JobInitManager(), "JobInitManagerThread");
      JobInitManagerThread.setDaemon(true);
      JobInitManagerThread.start();
    }
    public void initJob(JobInfo jobInfo, JobInProgress job) {
      if (!mockMode) {
    	  synchronized (jobInitQueue) {
    		  jobInitQueue.add(new InitJob(jobInfo, job));
			...
			while (true) {
				try {
					synchronized (jobInitQueue) {
						...
						initjob = jobInitQueue.remove(0);
					}
					threadPool.execute(initjob);
